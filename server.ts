    ...
    for (const item of msg.content) {
      if (item && item.type === "image_url" && item.image_url?.url) {
        ...
        if (
          typeof u === "string" &&
          imageCount < 3 &&
          (u.startsWith("https://") || u.startsWith("http://") || (u.startsWith("data:image/") && u.length < 500000)) &&
...
async function callOpenRouter(
  ...
  try {
    ...
    console.warn(`OpenRouter primary call (${targetModel}) failed:`, err.message);

    // Fallback 1: If hasImages and targetModel wasn't gemini-2.5-flash-lite or gemini-2.5-flash, try gemini-2.5-flash-lite
    if (hasImages && targetModel !== "google/gemini-2.5-flash-lite" && targetModel !== "google/gemini-2.5-flash") {
      try {
        console.log("Retrying vision prompt with google/gemini-2.5-flash-lite...");
        const response = await makeApiCall("google/gemini-2.5-flash-lite", processedMessages);
        if (options?.stream) return response;
        const data = await response.json();
        return data.choices?.[0]?.message;
      } catch (vErr: any) {
        console.warn("Vision fallback call with gemini-2.5-flash-lite failed:", vErr.message);
      }
    }

    // Fallback 2: Strip images and retry with text-only prompt using gemini-2.5-flash-lite or gemini-2.5-flash
    if (hasImages) {
      console.log("Retrying OpenRouter with text-only prompt...");
      const textOnlyMessages = processedMessages.map((msg) => {
        if (Array.isArray(msg.content)) {
          const textItem = msg.content.find((i: any) => i && i.type === "text");
          return { ...msg, content: textItem ? textItem.text : "" };
        }
        return msg;
      });

      try {
        const response = await makeApiCall("google/gemini-2.5-flash-lite", textOnlyMessages);
        if (options?.stream) return response;
        const data = await response.json();
        return data.choices?.[0]?.message;
      } catch (tErr: any) {
        console.warn("Text-only fallback with gemini-2.5-flash-lite failed, trying gemini-2.5-flash:", tErr.message);
        const response = await makeApiCall("google/gemini-2.5-flash", textOnlyMessages);
        if (options?.stream) return response;
        const data = await response.json();
        return data.choices?.[0]?.message;
      }
    ...
    // Fallback 3: Text-only model retry with alternative gemini model
    if (targetModel === "google/gemini-2.5-flash-lite") {
      console.log("Retrying text prompt with google/gemini-2.5-flash...");
      const response = await makeApiCall("google/gemini-2.5-flash", processedMessages);
      if (options?.stream) return response;
      const data = await response.json();
      return data.choices?.[0]?.message;
    }
...
app.post("/api/reseller/ai/generate-copy", async (req, res) => {
  try {
    const { productName, productDescription, platform, tone, price, profit, ageGroup } = req.body;

    const prompt = `أنت خبير تسويق إلكتروني متخصص في التجارة بالعمولة وملابس الأطفال بالجزائر (Kids Market Algeria).
اكتب منشور تسويقي جذب زبائن على منصة ${platform || "WhatsApp"}
منتج: ${productName || "ملابس أطفال"}
الفئة العمرية: ${ageGroup || "0-6 سنوات"}
الوصف والمميزات: ${productDescription || "نوعية رفيعة قطن 100%، مريحة وأنيقة"}
سعر البيع للزبون: ${price || "3500"} DZD
...
2. أهم المميزات (الراحة، القطن الممتاز، الأناقة)
3. السعر والتوصيل متاح لـ 58 ولاية والدفع عند الاستلام
4. دعوة مباشرة للشراء (أرسل رسالة أو اتصل الآن)
اجعل النطاق قصير ومناسب لرسالة واتساب أو منشور انستغرام/تيك توك.`;

    let generatedText = "";
    try {
      const resMsg = await callOpenRouter([
        { role: "user", content: prompt }
      ]);
      generatedText = resMsg?.content || "";
    } catch (aiErr) {
      console.error("OpenRouter API error in generate-copy:", aiErr);
    }

    if (!generatedText) {
      // High-quality Arabic fallback copy if API key is not configured or fails
      generatedText = `✨ ملابس أطفال ولا أروع لبطوسك الصغير! ✨
🛍️ ${productName || "طقم ملابس صيفي رائع للأطفال"} (${ageGroup || "من 0 إلى 6 سنوات"})

شياكة، خامة قطنية 100% ناعمة على بشرة ولدك/بنتك، ومناسبة لجميع المناسبات! 😍
...
app.post("/api/admin/ai/generate-product-description", async (req, res) => {
  try {
    ...
    if (Array.isArray(images) && images.length > 0) {
      for (const imgStr of images.slice(0, 5)) {
        if (typeof imgStr === "string" && imgStr.trim().length > 0) {
          const cleanStr = imgStr.trim();
          userMessageContent.push({
            type: "image_url",
            image_url: { url: cleanStr }
          });
        }
      }
    }

    const textPrompt = `أنت أخصائي كتابة محتوى وصفحات هبوط (Landing Page Copywriter) متخصص في صياغة الأوصاف الإقناعية الموجهة للزبائن بالسوق الجزائري.

الهدف الرئيسي: التعرف على المنتج بوضوح من الصور المحملة المرفقة، وكتابة وصف منتج كامل وإقناعي لصفحة الهبوط باستخدام هيكل صيغة PASO (المشكلة - الإثارة - الحل - النتيجة):

تفاصيل المنتج المعروفة:
- اسم المنتج الحالي: ${productName || "غير محدد (اعتمد على الصورة)"}
- التصنيف والفئة: ${category || "منتج تجاري"}
${targetAudience ? `- الجمهور المستهدف: ${targetAudience}` : ""}
${extraDetails ? `- ملاحظات إضافية: ${extraDetails}` : ""}

يرجى الالتزام بالتقسيم والتنسيق التالي بالضبط لصياغة الوصف بأسلوب صفحة الهبوط (Landing Page Copy):

🎯 **[عنوان رئيسي جذاب لصفحة الهبوط يعكس القيمة الاستثنائية]**

🚨 **1. المشكلة (Problem):**
(تحديد ألم أو الاحتياج الشديد للزبون، أو الصعوبة التي يواجهها يومياً في هذه الفئة)

🔥 **2. الإثارة والتهويل (Agitate):**
(تسليط الضوء على المعاناة من البدائل الرديئة والمغشوشة، وكيف تسبب إهدار الأموال والإحباط اليومي)

💡 **3. الحل المثالي (Solution):**
(تقديم هذا المنتج كحل رائع استناداً لتفاصيله الظاهرة بالصور، مع استعراض 3-4 مميزات رئيسية مثل الجودة، الأناقة، الاستخدام المريح)

✨ **4. النتيجة والعرض المغري (Outcome):**
(وصف شعور الزبون والتغير الإيجابي بعد اقتناء المنتج، مع التأكيد على: التوصيل السريع لـ 58 ولاية، الدفع عند الاستلام، والضمان قبل السداد).

اكتب بأسلوب عربي فصيح مبسط مع لمسة لهجة جزائرية محببة وإيموجيات معبرة.`;
    ...
    try {
      const resMsg = await callOpenRouter([
        { role: "user", content: userMessageContent.length > 1 ? userMessageContent : textPrompt }
      ]);
      generatedText = resMsg?.content || "";
    } catch (err: any) {
      console.error("OpenRouter API call error in generate-product-description:", err);
    }

    if (!generatedText) {
      // High-quality PASO Arabic fallback description if API key is not set or network fails
...
🚨 **1. المشكلة (Problem):**
هل تعبت من البحث عن ${catName} تجمع بين الجودة الفائقة والتصميم الجذاب دون التضحية بالراحة المتكاملة؟ معظم المنتجات المتوفرة بالسوق رديئة الصنع وتتلف سريعاً بعد استخدامات معدودة.

🔥 **2. الإثارة والتهويل (Agitate):**
إن إنفاق أموالك على منتجات منخفضة الجودة لا يسبب لك الإحباط فحسب، بل يجعلك تضطر للشراء والتكرار مراراً وتكراراً، مع المعاناة المستمرة من المظهر الباهت وعدم الرضا.

💡 **3. الحل المثالي (Solution):**
إليك **${pName}** المصمم بعناية فائقة ليكون خيارك الأول:
- 🌟 **تصميم عصري وجذاب:** يواكب أحدث الصيحات كما يظهر في الصور المرفقة.
- 🛡️ **خامة ممتازة ومقاومة:** مصنوع من أجود المواد لضمان المتانة واستدامة الاستخدام.
- ⚡ **أداء عملي مريح:** يمنحك التوازن المثالي بين الأناقة والاستخدام اليومي السلس.

✨ **4. النتيجة والعرض الإقناعي (Outcome):**
استمتع بالثقة المتميزة والمظهر الأنيق الذي يستحقه يومك!
🚚 **توصيل سريع ومضمون لـ 58 ولاية جزائرية.**
💵 **الدفع عند الاستلام** (تتسلم وتفحص منتجك قبل السداد).
📲 **لا تتردد! اطلب منتجك الآن واضمن العرض المباشر.**`;
    }
...
app.post("/api/admin/ai/generate-product-name", async (req, res) => {
  try {
    ...
    const textPrompt = `أنت أخصائي تسمية وتسويق المنتجات للتجارة الإلكترونية بالسوق الجزائري.
وظيفتك: التعرف البصري الفائق (Multimodal Vision) على صور المنتج المرفقة لاستخلاص:
1. الماركة أو البراند (Brand Name) - شرط حاسم ومهم جداً: يمنع منعاً باتاً ترجمة اسم الماركة إلى اللغة العربية! يجب ترك اسم البراند باللغة والأحرف الأجنبية الأصلية (إنجليزية أو فرنسية) كما هي مكتوبة على العلبة أو المنتج بالضبط (مثال: CeraVe, Eucerin, La Roche-Posay, Nike, Apple, Samsung, Anker, L'Oréal, Embryolisse, Chanel, etc.).
2. نوع المنتج ووظيفته وسعته الأساسية بلغة عربية تسويقية واضحة ومقنعة للزبون الجزائري.

اسم المنتج الحالي المقترح: ${currentName || "غير محدد"}
فئة المنتج: ${category || "عام"}

صيغة اسم المنتج المطلوب بالظبط:
[اسم البراند بالأحرف الأجنبية الأصلية] + [نوع المنتج ووظيفته الفائقة باللغة العربية]

أمثلة توضيحية:
- CeraVe كريم ترطيب وتغذية للبشرة الجافة والعادية 453g
- Eucerin سيروم مضاد للتصبغات للبشرة المشرقة 30ml
- Anker شاحن سريع ذكي بقوة 65 واط
- Nike حذاء رياضي كاجوال خفيف للمشي

تنبيه شديد: أعد الاسم المكتمل النهائي فقط بدون أي مقدمات أو شرح أو علامات تنصيص!`;
    ...
    try {
      const rawResMsg = await callOpenRouter([
        { role: "user", content: userMessageContent.length > 1 ? userMessageContent : textPrompt }
      ]);
      const rawRes = rawResMsg?.content || "";
      generatedName = rawRes.trim().replace(/^["'«»]+|["'«»]+$/g, "");
    } catch (err: any) {
      console.error("OpenRouter API call error in generate-product-name:", err);
    }
