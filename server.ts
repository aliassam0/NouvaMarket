...
// Delete order endpoint with real-time broadcast
app.delete("/api/reseller/orders/:id", (req, res) => {
  const orderId = req.params.id;
  const idx = serverOrders.findIndex((o) => o.id === orderId || o.trackingCode === orderId);
  if (idx !== -1) {
    const deleted = serverOrders.splice(idx, 1)[0];
    broadcastOrderUpdate({ type: "order_deleted", orderId });
    return res.json({ success: true, message: "تم حذف الطلبية بنجاح", deleted });
  }
  broadcastOrderUpdate({ type: "order_deleted", orderId });
  return res.json({ success: true, message: "الطلبية محذوفة مسبقاً" });
});
