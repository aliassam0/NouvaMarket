...
export function ProductEditModal({
  ...
  const handleGeneratePasoDescription = async () => {
    try {
      ...
      onShowToast('✨ جاري تحليل صور المنتج بالذكاء الاصطناعي وصياغة وصف PASO دقيق لصفحة الهبوط...', 'info');
      ...
      if (data.descriptionAr) {
        ...
        onShowToast('✔ تم التعرف على المنتج وتوليد وصف صفحة الهبوط الإقناعي (PASO) بنجاح!', 'success');
      } else {
          ...
          <div className="p-3.5 rounded-2xl bg-slate-950 border border-purple-900/40 space-y-2.5">
            ...
            {productData.descriptionAr && (productData.descriptionAr.includes('PASO') || productData.descriptionAr.includes('المشكلة')) && (
              <div className="flex flex-wrap items-center gap-1.5 pt-1 text-[10px] border-t border-slate-900">
                ...
                <span className="px-2 py-0.5 rounded-md bg-purple-500/10 border border-purple-500/30 text-purple-300 font-bold flex items-center gap-1">
                  💡 3. الحل (Solution)
                </span>
                <span className="px-2 py-0.5 rounded-md bg-purple-500/10 border border-purple-500/30 text-purple-300 font-bold flex items-center gap-1">
