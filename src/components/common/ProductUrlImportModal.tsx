...
export function ProductUrlImportModal({
  ...
  const handleConfirmImport = () => {
    const productsToImport = extractedProducts
      ...
      .map((p, pIdx) => ({
        ...p,
        id: `p-${Date.now()}-${pIdx + 1}-${Math.random().toString(36).substring(2, 8)}`,
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 mt-0.5">
                استخراج الصور، الاسم، الوصف، جميع الألوان والمقاسات (Variants)، والأسعار بالكامل تلقائياً
              </p>
            </div>
          </div>
          <button
              ...
              {extractedProducts.map((p, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Colors & Sizes (Variants) Extracted Preview */}
                    {Array.isArray(p.variants) && p.variants.length > 0 && (
                      <div className="pt-2 border-t border-slate-800/80 bg-slate-900/40 p-2.5 rounded-xl space-y-2">
                        <div className="flex items-center justify-between text-[10px]">
                          <span className="font-extrabold text-purple-400 flex items-center gap-1.5">
                            <Layers className="w-3.5 h-3.5 text-purple-400" />
                            <span>الألوان والمقاسات المستردة ({p.variants.length} متغير)</span>
                          </span>
                          <span className="text-[10px] text-slate-400 font-mono">
                            إجمالي المخزون المسترد: {p.variants.reduce((sum, v) => sum + (Number(v.stockCount) || 0), 0)} قطعة
                          </span>
                        </div>

                        {(() => {
                          const colorsList = Array.from(new Set(p.variants.map((v) => v.color).filter(Boolean)));
                          const sizesList = Array.from(new Set(p.variants.map((v) => v.size).filter(Boolean)));
                          return (
                            <div className="space-y-1.5">
                              {colorsList.length > 0 && (
                                <div className="flex items-center gap-1.5 flex-wrap">
                                  <span className="text-[9px] text-slate-400 font-bold">الألوان ({colorsList.length}):</span>
                                  {colorsList.map((cName) => {
                                    const sampleVar = p.variants.find((v) => v.color === cName);
                                    return (
                                      <span
                                        key={cName}
                                        className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-slate-800 border border-slate-700 text-[10px] font-bold text-slate-200"
                                      >
                                        <span
                                          className="w-2.5 h-2.5 rounded-full border border-slate-600 shrink-0"
                                          style={{ backgroundColor: sampleVar?.colorHex || '#2563eb' }}
                                        />
                                        <span>{cName}</span>
                                      </span>
                                    );
                              ...
                              {sizesList.length > 0 && (
                                <div className="flex items-center gap-1.5 flex-wrap">
                                  <span className="text-[9px] text-slate-400 font-bold">المقاسات ({sizesList.length}):</span>
                                  {sizesList.map((sName) => (
                                    <span
                                      key={sName}
                                      className="px-2 py-0.5 rounded-md bg-purple-950/60 border border-purple-800/60 text-[10px] font-mono font-extrabold text-purple-300"
                                    >
                                      {sName}
                                    </span>
                                  ))}
