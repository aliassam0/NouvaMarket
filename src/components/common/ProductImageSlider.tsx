...
export function ProductImageSlider({
  ...
  // Scroll to selectedImage if provided
  useEffect(() => {
    if (!selectedImage) return;
    const matchIdx = safeImages.findIndex((img) => img === selectedImage);
    if (matchIdx !== -1) {
      ...
      {showDownloadBtn && (
        <div className="flex items-center justify-between gap-2 px-1 py-0.5">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-gradient-to-r from-purple-600 via-purple-600 to-purple-700 hover:from-purple-500 hover:to-purple-500 text-white font-extrabold text-xs shadow-md transition transform active:scale-95 cursor-pointer disabled:opacity-50 shrink-0"
          >
              ...
              {showDownloadBtn && (
                <>
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs shadow-lg transition active:scale-95 cursor-pointer"
                  ...
                  {safeImages.length > 1 && (
                    <button
                      ...
                      onClick={handleDownloadAll}
                      disabled={isDownloading}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs shadow-lg transition active:scale-95 cursor-pointer disabled:opacity-50"
            ...
            >
              {safeImages.map((img, idx) => (
                <button
                  ...
                  className={`w-14 h-14 rounded-xl overflow-hidden border-2 transition shrink-0 cursor-pointer ${
                    activeIndex === idx
                      ? 'border-purple-400 ring-2 ring-purple-500/40 scale-105 opacity-100'
