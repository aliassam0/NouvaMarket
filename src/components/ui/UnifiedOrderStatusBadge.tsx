...
export function UnifiedOrderStatusBadge({ order }: UnifiedOrderStatusBadgeProps) {
  ...
  return (
    <div className="p-3.5 rounded-2xl bg-gradient-to-r from-slate-50 via-violet-50/50 to-purple-50/50 dark:from-slate-900 dark:via-slate-800/80 dark:to-slate-800/80 border border-violet-200/80 dark:border-violet-900/60 shadow-xs space-y-2.5 text-xs">
      {/* Header */}
      ...
      <div className="flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400 px-1 pt-0.5">
        <span>كود التتبع: <strong className="font-mono text-purple-600 dark:text-purple-400 font-extrabold">{trackingCode}</strong></span>
      </div>
    </div>
  );
}
