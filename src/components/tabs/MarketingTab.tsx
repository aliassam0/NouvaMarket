    ...
    try {
      if (platform === 'LandingPage_AIDA') {
        ...
        if (result.success && result.descriptionAr) {
          setGeneratedText(result.descriptionAr);
          onShowToast('✔ تم التعرف على صور المنتج وتوليد وصف صفحة الهبوط بصيغة AIDA!');
          return;
        } else {
          onShowToast(result.error || 'تعذر توليد وصف صفحة الهبوط');
          return;
        }
      }

      const result = await generateMarketingCopySmart({
        productName: selectedProduct.nameAr,
      ...
      if (result.success && result.generatedText) {
        ...
        onShowToast(result.error || 'حدث خطأ أثناء الاتصال بـ Gemini AI');
      }
    } catch (e) {
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-1.5 text-xs font-bold">
            {(
              [
                ...
                { id: 'LandingPage_AIDA', label: '✨ صفحة هبوط (AIDA)' },
              ] as const
            ).map((p) => (
              <button
                key={p.id}
