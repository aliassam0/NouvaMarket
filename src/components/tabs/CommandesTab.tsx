...
import { Package, Search, Clock, CheckCircle2, Truck, AlertTriangle, XCircle, Share2, Eye, RefreshCw, Printer, ShieldCheck, Code, Check, Pencil, Lock, Trash2, Send, Zap, Radio, Globe, Key, FileSearch, AlertCircle, MapPin, Store } from 'lucide-react';
...
export function CommandesTab() {
  ...
  const [isPullingExternalOrders, setIsPullingExternalOrders] = useState(false);

  const handlePullExternalOrders = async () => {
    setIsPullingExternalOrders(true);
    const res = await pullOrdersFromStores();
    setIsPullingExternalOrders(false);
  ...
  const getStatusBadge = (status: OrderStatus, textAr: string, textFr: string, order?: Order) => {
    ...
    if (order?.adminConfirmed || status === 'CONFIRMED') {
      return (
        <span className="px-2.5 py-1 rounded-full bg-purple-100 text-purple-800 dark:bg-purple-950/80 dark:text-purple-300 text-[11px] font-bold flex items-center gap-1 border border-purple-200 dark:border-purple-800">
    ...
    switch (status) {
      case 'DELIVERED':
        return (
          <span className="px-2.5 py-1 rounded-full bg-purple-100 text-purple-800 dark:bg-purple-950/80 dark:text-purple-300 text-[11px] font-bold flex items-center gap-1">
  ...
  return (
    <div className="flex-1 pb-24 overflow-y-auto p-4 text-slate-900 dark:text-slate-100 space-y-4">
      ...
      {actionNotice && (
        <div className="fixed top-4 inset-x-4 z-50 p-3 rounded-2xl bg-purple-600 text-white font-bold text-xs shadow-2xl flex items-center justify-between border border-purple-400 animate-bounce">
          <div className="flex items-center gap-2">
            <CheckCircle2 className="w-5 h-5" />
            <span>{actionNotice}</span>
          </div>
          <button onClick={() => setActionNotice(null)} className="p-1 hover:bg-purple-700 rounded-lg">
        ...
        <div className="space-y-3">
          {filtered.map((order, idx) => {
            ...
            if (order.status === 'DELIVERED' || order.situation === 'Livre') {
              cardBorderStyle = 'border-2 border-purple-200 dark:border-purple-900/80 border-s-4 border-s-purple-500';
              ...
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5">
                <div>
                  <div className="flex flex-wrap items-center gap-1.5">
                    ...
                    {(order.externalStorePlatform || order.source === 'shopify' || order.source === 'youcan' || order.source === 'woocommerce') && (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-violet-100 dark:bg-violet-950/80 text-violet-700 dark:text-violet-300 text-[10px] font-black border border-violet-200 dark:border-violet-800 shadow-2xs">
                        <span>
                          {order.externalStorePlatform === 'shopify' || order.source === 'shopify'
                            ? '🛍️ شوبيفاي'
                            : order.externalStorePlatform === 'youcan' || order.source === 'youcan'
                            ? '🛒 يوكان'
                            : '⚡ ووكومرس'}
                        </span>
                        {order.externalOrderNumber && <span>{order.externalOrderNumber}</span>}
                        {order.externalStoreName && <span className="opacity-75">• {order.externalStoreName}</span>}
                      </span>
                    )}
                  </div>
                ...
                <div className="p-3 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 space-y-2 text-xs">
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <button
                      ...
                      className="flex-1 py-2 px-3 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-extrabold text-xs flex items-center justify-center gap-1.5 shadow-xs transition cursor-pointer"
                    >
                ...
                {order.items.map((it, idx) => (
                  <div key={idx} className="flex justify-between items-center gap-2">
                    <div className="flex items-center gap-1.5 min-w-0">
                      {it.isUpsell ? (
                        <span className="px-1.5 py-0.5 rounded-md bg-amber-500/15 text-amber-700 dark:text-amber-400 text-[10px] font-black border border-amber-500/30 shrink-0">
                          ⚡ Upsell
                        </span>
                      ) : it.isCrossSell ? (
                        <span className="px-1.5 py-0.5 rounded-md bg-amber-500/15 text-amber-700 dark:text-amber-400 text-[10px] font-black border border-amber-500/30 shrink-0">
                      ...
                      <span className="line-clamp-1 font-medium">
                        {it.productName.replace(/^\[(?:عرض إضافي|Cross-sell|Grossell)\]\s*/, '')} {it.variantSize && !it.isUpsell ? `(${it.variantSize})` : ''} × {it.quantity}
                      </span>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {!order.adminConfirmed ? (
                      <button
                        ...
                        className="col-span-2 py-2 px-3 rounded-xl bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold transition flex items-center justify-center gap-1.5 shadow-xs"
                      >
              ...
              <div className="flex items-center justify-between pt-1">
                <div>
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {order.platformResellerFee && order.platformResellerFee > 0 ? (
                      <span className="text-[10px] text-purple-600 dark:text-purple-400 font-bold bg-purple-50 dark:bg-purple-950/60 px-1.5 py-0.5 rounded-md border border-purple-200 dark:border-purple-800">
                        صافي بعد اقتطاع المنصة ({order.resellerFeePercent || ''}% = {order.platformResellerFee} دج)
                      </span>
                    ) : null}
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/80 backdrop-blur-xs flex justify-center items-center p-4">
          <div className="w-full max-w-lg bg-white dark:bg-slate-900 text-slate-900 dark:text-white rounded-3xl p-5 shadow-2xl space-y-4 relative border border-slate-200 dark:border-slate-800 max-h-[90vh] overflow-y-auto">
            ...
            {updatedSuccessLabelUrl && (
              <div className="p-3 rounded-2xl bg-purple-100 text-purple-900 text-xs font-bold space-y-2">
                <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="w-full py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-xl font-extrabold text-xs flex items-center justify-center gap-1.5 shadow-xs"
                >
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/80 backdrop-blur-xs flex justify-center items-center p-4">
          <div className="w-full max-w-md bg-slate-900 text-white rounded-3xl p-5 shadow-2xl space-y-4 relative border border-slate-800 max-h-[85vh] overflow-y-auto">
            ...
            <div className="space-y-1">
              <label className="text-[11px] font-bold text-purple-400 block">Headers المطلوبة:</label>
...
Content-Type: application/json`}
            ...
            <div className="space-y-1">
              <label className="text-[11px] font-bold text-purple-400 block">1. إضافة طردجديد: POST /Api_v1/Colis</label>
              <pre className="p-3 rounded-xl bg-slate-950 text-[10px] font-mono text-purple-300 overflow-x-auto border border-slate-800">
{JSON.stringify({
  Colis: [
    {
      Echange: 0,
  ...
  Colis: [
    {
      ...
      Date_Livrée: null
            ...
            <div className="space-y-1">
              <label className="text-[11px] font-bold text-purple-400 block">6. الاستعلام بكود التتبع: GET /Api_v1/Colis/Tracking/{selectedApiDetails.trackingCode || 'TC317LHJ'}</label>
              <pre className="p-3 rounded-xl bg-slate-950 text-[10px] font-mono text-purple-300 overflow-x-auto border border-slate-800">
...
Headers: { key: "3490e731e3db4d8c841991987d3cab0f", token: "b8386c67-f0ce-4ce5-bc3b-cf3246a90819" }`}
            ...
            <div className="space-y-1">
              <label className="text-[11px] font-bold text-purple-400 block">7. استرجاع قائمة طرود محددة: POST /Api_v1/Colis/Liste</label>
              <pre className="p-3 rounded-xl bg-slate-950 text-[10px] font-mono text-purple-300 overflow-x-auto border border-slate-800">
{JSON.stringify({
  Colis: [
    { Tracking: selectedApiDetails.trackingCode || "TC317LHJ" }
  ...
  Historique: [
    {
      ...
      Commentaire: selectedApiDetails.noteFournisseur || "سجل إجراءات الطرد"
            ...
            <div className="space-y-1">
              <label className="text-[11px] font-bold text-purple-400 block">11. قائمة البلديات المسجلة: GET /Api_v1/Commune/31</label>
              <pre className="p-3 rounded-xl bg-slate-950 text-[10px] font-mono text-purple-300 overflow-x-auto border border-slate-800">
{JSON.stringify({
  Quota: {
    Consommer_1min: 1,
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/80 backdrop-blur-xs flex justify-center items-center p-4">
          <div className="w-full max-w-sm bg-white dark:bg-slate-900 rounded-3xl p-5 shadow-2xl space-y-4 relative">
            ...
            <div className="space-y-4 ps-2 border-s-2 border-violet-500 text-xs my-3">
              ...
              <div className="ps-3 relative">
                <div className="w-3 h-3 bg-purple-500 rounded-full absolute -start-[19px] top-0.5 border-2 border-white" />
          ...
          <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-2xl space-y-4 text-slate-100 relative max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3 bg-slate-950 p-4 rounded-2xl border border-slate-800/80 text-xs">
              ...
              <div className="space-y-1">
                ...
                <div className="p-2.5 rounded-xl bg-slate-900 border border-slate-800 text-purple-300 font-mono text-xs flex items-center justify-between overflow-x-auto">
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                ...
                {testWebhookResponse && (
                  <span className="text-[10px] font-extrabold text-purple-400 bg-purple-950 px-2 py-0.5 rounded-full border border-purple-800">
                    ✔ Test OK (200)
                  </span>
                )}
              </div>
