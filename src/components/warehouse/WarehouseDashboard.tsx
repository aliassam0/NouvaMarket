...
import { fetchLiveTrackingFromCourier } from '../../lib/deliveryApiManager';
...
export function WarehouseDashboard({ onShowToast }: WarehouseDashboardProps) {
  ...
  const [ecomTestLoading, setEcomTestLoading] = useState(false);
  const [ecomTestResult, setEcomTestResult] = useState<any>(null);
  ...
  const [couriers, setCouriersState] = useState<CourierPartner[]>(() =>
    getStoredCouriers(user?.id || user?.email)
  );

  useEffect(() => {
    if (user?.id || user?.email) {
      setCouriersState(getStoredCouriers(user.id || user.email));
    }
  }, [user?.id, user?.email]);

  const setCouriers = (
    action: CourierPartner[] | ((prev: CourierPartner[]) => CourierPartner[])
  ) => {
    setCouriersState((prev) => {
      const next = typeof action === 'function' ? action(prev) : action;
      saveStoredCouriers(next, user?.id || user?.email);
      return next;
    });
  };
  ...
  // E-com Delivery API v2 Handlers
  const handleTestEcomConnection = async (courier: CourierPartner) => {
    setEcomTestLoading(true);
    setEcomTestResult(null);
    try {
      const client = new EcomDeliveryApiClient(courier.apiKey, courier.apiSecret);
      const res = await client.testConnection();
      if (res.ok && res.data) {
        setEcomTestResult(res.data);
        onShowToast(`✅ نجاح الاتصال بـ E-com Delivery API v2! رقم المورد: #${res.data.id_fournisseur} - ${res.data.nom_fournisseur}`, 'success');
      } else {
        onShowToast('تعذر الاتصال بـ E-com API v2. يرجى التحقق من المفتاح والتوكن', 'error');
      }
    } catch (err) {
      onShowToast('خطأ أثناء اختبار مفاتيح E-com API v2', 'error');
    } finally {
      setEcomTestLoading(false);
    }
  };

  const handleFetchEcomSummary = async (courier: CourierPartner) => {
    try {
      const client = new EcomDeliveryApiClient(courier.apiKey, courier.apiSecret);
      const stats = await client.getSummaryStats();
      setEcomSummaryData(stats);
      setShowEcomSummaryModal(true);
      onShowToast('تم جلب إحصائيات الأداء الحية من E-com Delivery بنجاح!', 'success');
    } catch (e) {
      onShowToast('تعذر جلب ملخص E-com API v2', 'error');
    }
  };

  const handleFetchEcomPaiements = async (courier: CourierPartner) => {
    try {
      const client = new EcomDeliveryApiClient(courier.apiKey, courier.apiSecret);
      const res = await client.getPaiements();
      setEcomPaiementsData(res.items || []);
      setShowEcomPaiementsModal(true);
      onShowToast(`تم جلب ${res.items?.length || 0} كشف مستحقات وحوالات من E-com Delivery`, 'success');
    } catch (e) {
      onShowToast('تعذر جلب كشوفات مدفوعات E-com API v2', 'error');
    }
  };
  ...
  return (
    <div className="flex-1 pb-24 overflow-y-auto p-2.5 sm:p-4 text-slate-900 dark:text-slate-100 space-y-4 max-w-7xl mx-auto overflow-x-hidden w-full min-w-0">
        ...
        <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-5 xl:grid-cols-10 gap-2 pt-3 border-t border-purple-800/60 relative z-10">
          ...
          >
            ...
            <div className="mt-1 flex items-baseline justify-between">
              ...
              <span className="text-[9px] text-purple-200 font-medium">دج</span>
            </div>
          </div>
      ...
      {/* E-COM DELIVERY API HIGHLIGHT BANNER FOR SUPPLIERS */}
      <div className="p-4 rounded-3xl bg-gradient-to-r from-slate-900 via-sky-950 to-purple-950 text-white border-2 border-sky-500/80 shadow-xl flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-sky-500/20 border border-sky-400/50 flex items-center justify-center shrink-0">
            <Truck className="w-6 h-6 text-sky-400 animate-bounce" />
          </div>
          <div className="space-y-1 text-right">
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-0.5 rounded-full bg-purple-500/30 text-purple-300 font-mono text-[10px] font-black border border-purple-400/40">
                جديد - E-com Delivery API v2
              </span>
              <span className="text-xs font-mono text-sky-300 font-bold">تحديث الربط آلياً</span>
            </div>
            <h3 className="font-black text-sm text-white">
              تم إضافة وتفعيل ربط شركات التوصيل المستقلة (E-com Delivery API) لحسابك!
            </h3>
            <p className="text-xs text-slate-300">
              يمكنك الآن إجراء اختبار الاتصال السريع، طلب سائق الجمع (Ramassage)، ومتابعة نسب التوصيل الحية وحوالاتك المالية.
            </p>
          </div>
        </div>
        <button
          onClick={() => setActiveTab('couriers')}
          className="px-5 py-3 rounded-2xl bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-400 hover:to-blue-500 text-white font-black text-xs shadow-lg active:scale-95 transition cursor-pointer shrink-0 flex items-center gap-2 border border-sky-300/40"
        >
          <span>🚚 فتح كنسول شركات التوصيل والـ API</span>
                  ...
                  >
                    ...
                    <div className="flex flex-wrap justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-2 gap-2">
                      <div className="flex items-center gap-2.5">
                        ...
                        {isSentToShipping && (
                          <span className="px-2.5 py-0.5 rounded-full bg-purple-100 text-purple-800 dark:bg-purple-950 dark:text-purple-300 text-[10px] font-bold">
                    ...
                    <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-2 pt-2 border-t border-slate-100 dark:border-slate-800 text-xs font-bold">
                      ...
                      <button
                        ...
                        className="py-2 px-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white flex items-center justify-center gap-1.5 shadow-xs transition cursor-pointer"
                      >
      ...
      {activeTab === 'financial' && (() => {
        const totalSales = orders.filter((o) => o.status === 'DELIVERED').reduce((acc, o) => acc + (o.totalAmount || 0), 0);
        const nouvaCommission = Math.round(totalSales * 0.05);
        const marketerCommissions = orders.filter((o) => o.status === 'DELIVERED').reduce((acc, o) => acc + (o.commissionDzd || 0), 0);
        const totalDuesOwed = nouvaCommission + marketerCommissions;
        const paidAmount = supplierProfile.paidAmountDzd || 0;
        const remainingUnpaid = Math.max(totalDuesOwed - paidAmount, 0);
        const lastPayment = supplierProfile.lastPaymentDate || 'لا توجد دفعات بعد';
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="p-4 bg-gradient-to-r from-purple-50 to-purple-50 dark:from-purple-950/40 dark:to-purple-950/40 border border-purple-200 dark:border-purple-800/60 rounded-2xl text-xs text-slate-800 dark:text-slate-200 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xs">
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-1">
                  <h4 className="font-black text-sm text-slate-900 dark:text-white flex items-center gap-2">
                    ...
                    <span className="px-2 py-0.5 rounded-full bg-purple-100 text-purple-800 dark:bg-purple-900/60 dark:text-purple-300 text-[10px] font-mono">
                      COD Supplier Model
                    </span>
                  </h4>
              ...
              <button
                ...
                className="w-full md:w-auto px-5 py-2.5 bg-purple-600 hover:bg-purple-500 text-white font-black text-xs rounded-xl shadow-md transition flex items-center justify-center gap-2 shrink-0 cursor-pointer"
              >
                <Plus className="w-4 h-4" />
            ...
            <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="p-4 rounded-xl bg-purple-50/50 dark:bg-purple-950/20 border border-purple-200 dark:border-purple-800/40 space-y-2 md:col-span-2 lg:col-span-1">
                  <span className="text-xs font-extrabold text-purple-800 dark:text-purple-300 block">3. الحالة الحالية للتسوية</span>
                  <div className="space-y-1.5 text-xs">
                    <div className="flex justify-between text-slate-600 dark:text-slate-300">
                      <span>المبلغ المسدد سابقاً:</span>
                      <span className="font-mono font-black text-purple-600 dark:text-purple-400">{paidAmount.toLocaleString()} DA</span>
                    </div>
                    <div className="flex justify-between text-slate-900 dark:text-white font-extrabold pt-1 border-t border-purple-200 dark:border-purple-800">
                  ...
                  <button
                    ...
                    className="w-full mt-2 py-2 bg-purple-600 hover:bg-purple-500 text-white font-black text-xs rounded-lg transition flex items-center justify-center gap-1.5 cursor-pointer"
                  >
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                <h4 className="font-black text-sm text-slate-900 dark:text-white flex items-center gap-2">
                  ...
                  <span className="px-2.5 py-0.5 rounded-full bg-purple-100 text-purple-800 dark:bg-purple-900/60 dark:text-purple-300 text-[10px] font-extrabold font-mono">
                    مستقل وخاص بحساب المورد #{user?.id || user?.email || 'sup-01'}
                  </span>
                </h4>
                <p className="text-slate-600 dark:text-slate-300 text-xs leading-relaxed max-w-3xl">
                  لكل مورد إعدادات ومفاتيح API خاصة بشركات التوصيل (Ecom Delivery, Yalidine, Maystro, ZR Express...) منفصلة كلياً عن البائعين والموردين الآخرين. يتم حفظ واستعمال مفاتيح الربط ومتابعة حالة الاتصال الخاصة بحسابك فقط عند شحن ومعالجة طلباتك.
                </p>
              </div>
            </div>
          ...
          <div className="space-y-3">
            {couriers.length === 0 ? (
              <div className="p-8 text-center bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-300 dark:border-slate-800 space-y-4">
                <div className="w-12 h-12 rounded-2xl bg-sky-50 dark:bg-sky-950/80 text-sky-600 dark:text-sky-400 flex items-center justify-center mx-auto shadow-xs">
                  <Truck className="w-6 h-6" />
                </div>
                <div className="max-w-md mx-auto space-y-1">
                  <h4 className="font-black text-sm text-slate-900 dark:text-white">لا توجد شركات توصيل مرتبطة بحسابك حالياً</h4>
                  <p className="text-xs text-slate-500 leading-relaxed">
                    يمكنك التحكم كلياً في ربط منصة وشركة التوصيل الخاصة بك (Yalidine Express, Maystro, ZR Express, Ecom Delivery, EcoTrack, Nord&Sud) وإدخال مفاتيح الـ API ومتابعة الاتصال والتسليم.
                  </p>
                </div>
                <button
                  onClick={() => setIsAddingCourierModalOpen(true)}
                  className="px-5 py-2.5 rounded-2xl bg-sky-600 hover:bg-sky-500 text-white font-black text-xs inline-flex items-center gap-2 shadow-md transition cursor-pointer"
                >
                  <Plus className="w-4 h-4" />
                  <span>إضافة شركة توصيل جديدة لحسابك</span>
                </button>
              </div>
            ) : (
              couriers.map((c) => (
                ...
                >
                      ...
                      <div>
                        <h4 className="font-black text-base text-slate-900 dark:text-white flex items-center gap-2">
                          ...
                          {c.isDisabled ? (
                            ...
                            <span className="px-2.5 py-0.5 rounded-lg bg-purple-100 dark:bg-purple-950 text-purple-800 dark:text-purple-300 font-black text-[10px]">
                              ✓ مفعّلة وشغالة
                            </span>
                          )}
                        </h4>
                  ...
                  {/* E-com Delivery API v2 Developer Control Bar */}
                  {(c.id === 'cour-ecom' || c.name.toLowerCase().includes('ecom')) && (
                    <div className="mt-4 p-4 rounded-2xl bg-slate-950 text-white border border-sky-800/60 space-y-3">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-slate-800">
                        <div className="flex items-center gap-2">
                          <span className="w-2.5 h-2.5 rounded-full bg-purple-400 animate-pulse"></span>
                          <span className="font-extrabold text-xs text-sky-400 font-mono">E-com Delivery API v2 Developer Console</span>
                          <span className="px-2 py-0.5 rounded-md bg-sky-950 border border-sky-700/80 text-sky-300 text-[10px] font-mono">
                            https://ecom-dz.com/api_v2
                          </span>
                        </div>
                        <span className="text-[10px] text-slate-400 font-mono">Headers: X-API-Key + X-API-Token</span>
                      </div>

                      {/* Quotas Breakdown */}
                      <div className="grid grid-cols-3 gap-2 text-center text-[10px] font-mono">
                        <div className="p-2 rounded-xl bg-slate-900 border border-slate-800">
                          <span className="text-slate-400 block">Requêtes / min</span>
                          <strong className="text-purple-400 font-bold">50 max</strong>
                        </div>
                        <div className="p-2 rounded-xl bg-slate-900 border border-slate-800">
                          <span className="text-slate-400 block">Requêtes / heure</span>
                          <strong className="text-purple-400 font-bold">2,000 max</strong>
                        </div>
                        <div className="p-2 rounded-xl bg-slate-900 border border-slate-800">
                          <span className="text-slate-400 block">Requêtes / jour</span>
                          <strong className="text-purple-400 font-bold">20,000 max</strong>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-2 pt-1">
                        <button
                          onClick={() => handleTestEcomConnection(c)}
                          disabled={ecomTestLoading}
                          className="px-3 py-1.5 rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-bold text-xs flex items-center gap-1.5 transition cursor-pointer"
                        >
                          <RefreshCw className={`w-3.5 h-3.5 ${ecomTestLoading ? 'animate-spin' : ''}`} />
                          <span>اختبار الاتصال بالـ API (GET /test)</span>
                        </button>
                        ...
                        >
                          ...
                          <span>طلب سائق للجمع (Demande de Ramassage)</span>
                        </button>

                        <button
                          onClick={() => handleFetchEcomSummary(c)}
                          className="px-3 py-1.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs flex items-center gap-1.5 transition cursor-pointer"
                        >
                          <BarChart3 className="w-3.5 h-3.5" />
                          <span>نسب وتلخيص الأداء (GET /colis/resume)</span>
                        </button>

                        <button
                          onClick={() => handleFetchEcomPaiements(c)}
                          className="px-3 py-1.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs flex items-center gap-1.5 transition cursor-pointer"
                        >
                          <Wallet className="w-3.5 h-3.5" />
                          <span>كشوفات المستحقات (GET /paiements)</span>
                        </button>
                      </div>

                      {/* Test Connection Output */}
                      {ecomTestResult && (
                        <div className="p-2.5 rounded-xl bg-purple-950/80 border border-purple-700/80 text-[11px] text-purple-300 font-mono flex items-center justify-between">
                          <span>
                            ✓ تم المراسلة والربط مع شركة التوصيل E-com: [Nom: <strong>{ecomTestResult.nom_fournisseur}</strong> | ID Fournisseur: #{ecomTestResult.id_fournisseur}]
                          </span>
                          <span className="text-[10px] text-purple-400 font-bold">200 OK</span>
                        </div>
                      )}
                    </div>
                  )}
          ...
          <div className="w-full max-w-sm bg-white text-slate-900 rounded-3xl p-4 sm:p-5 shadow-2xl space-y-4 relative border-2 border-slate-900 my-auto max-h-[90vh] overflow-y-auto">
            ...
            <div className="border-b-2 border-slate-900 pb-3 text-center space-y-1">
              <div className="flex items-center justify-between">
                <span className="font-black text-sm tracking-wider text-purple-700">
            ...
            <form onSubmit={handlePerformSupplierPayment} className="space-y-3 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl font-mono font-black text-sm text-purple-600"
                />
              </div>
              ...
              <div className="pt-2 flex justify-end gap-2">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-black flex items-center gap-1.5 shadow-md transition cursor-pointer"
                >
                  <CheckCircle2 className="w-4 h-4" />
