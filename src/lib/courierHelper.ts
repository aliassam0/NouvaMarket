...
/**
 * Centrally managed delivery companies.
 * The Admin controls all delivery company integrations, contracts, and API keys.
 * Suppliers use the Admin-approved delivery partners without managing their keys or finances directly.
 */
...
export function saveStoredCouriers(couriers: CourierPartner[], supplierId?: string): void {
  try {
    ...
    window.dispatchEvent(new CustomEvent('nouva_couriers_updated', { detail: couriers }));
  } catch (e) {
