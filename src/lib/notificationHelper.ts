export interface AppNotification {
  ...
  recipientRole?: 'seller' | 'admin' | 'warehouse' | 'confirmer' | 'all';
}
...
const STORAGE_KEY_CONFIRMER = 'confirmer_notifications_v1';
...
const INITIAL_CONFIRMER_NOTIFICATIONS: AppNotification[] = [];

export function getStoredNotifications(role: 'seller' | 'admin' | 'warehouse' | 'confirmer' = 'seller'): AppNotification[] {
  ...
  if (role === 'admin') {
    ...
    key = STORAGE_KEY_CONFIRMER;
    initial = INITIAL_CONFIRMER_NOTIFICATIONS;
  }
...
export function saveStoredNotifications(
  ...
  role: 'seller' | 'admin' | 'warehouse' | 'confirmer' = 'seller'
): void {
  let key = STORAGE_KEY_SELLER;
  ...
  if (role === 'admin') {
    ...
    key = STORAGE_KEY_CONFIRMER;
    eventName = 'confirmer_notifications_updated';
  }
...
export function addNotification(
  ...
  if (targetRole === 'confirmer' || targetRole === 'all') {
    const confirmerNotifs = getStoredNotifications('confirmer');
    saveStoredNotifications([newNotif, ...confirmerNotifs], 'confirmer');
  }
...
export function addConfirmerNotification(
  item: Omit<AppNotification, 'id' | 'createdAt' | 'timestamp' | 'isRead' | 'recipientRole'>
): void {
  addNotification({ ...item, recipientRole: 'confirmer' });
}

export function markNotificationAsRead(id: string, role: 'seller' | 'admin' | 'warehouse' | 'confirmer' = 'seller'): void {
...
export function markAllNotificationsAsRead(role: 'seller' | 'admin' | 'warehouse' | 'confirmer' = 'seller'): void {
  const notifs = getStoredNotifications(role);
  const updated = notifs.map((n) => ({ ...n, isRead: true }));
  saveStoredNotifications(updated, role);
}

export function clearAllNotifications(role: 'seller' | 'admin' | 'warehouse' | 'confirmer' = 'seller'): void {
  saveStoredNotifications([], role);
}

export function getUnreadNotificationsCount(role: 'seller' | 'admin' | 'warehouse' | 'confirmer' = 'seller'): number {
  const notifs = getStoredNotifications(role);
