...
const DELETED_PRODUCTS_KEY = 'nouva_deleted_products_ids_v1';

/**
 * Returns set of product IDs permanently deleted by user/admin to prevent resurrection
 */
export function getDeletedProductIds(): Set<string> {
  try {
    ...
    console.error('Error reading deleted products set:', e);
  }
  return new Set<string>();
}

/**
 * Permanently mark a product as deleted in the blacklist and inform server
 */
export function markProductDeleted(productId: string): void {
  try {
    ...
    console.error('Error blacklisting deleted product:', e);
  }

  // Also call backend API to delete permanently from server storage
  try {
    fetch(`/api/products/${encodeURIComponent(productId)}`, {
      method: 'DELETE',
    }).catch((err) => {
      console.warn('Network issue while calling DELETE /api/products:', err);
    });
  } catch {}
}

/**
 * Unmark a product from deleted list (used if a product is re-created with the same ID)
 */
export function unmarkProductDeleted(productId: string): void {
  try {
    const current = getDeletedProductIds();
    current.delete(String(productId));
    localStorage.setItem(DELETED_PRODUCTS_KEY, JSON.stringify(Array.from(current)));
  } catch (e) {
    console.error('Error removing product from deleted set:', e);
  }
}

/**
 * Completely removes a product from local storage, blacklists its ID, and deletes it from server
 */
export function deleteStoredProduct(productId: string): void {
  markProductDeleted(productId);
  try {
    const current = getStoredProducts();
    const updated = current.filter((p) => p && p.id !== productId);
    localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(updated));
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new Event('products_updated'));
    }
  } catch (e) {
    console.error('Error deleting product from storage:', e);
  }
}

/**
 * Ensures all product IDs in a list are strictly unique to prevent duplicate React keys,
 * without ever deleting or dropping any products or user data.
 */
export function ensureUniqueProductIds(products: Product[]): { products: Product[]; hadDuplicates: boolean } {
  if (!Array.isArray(products)) return { products: [], hadDuplicates: false };
  const seenIds = new Set<string>();
  let hadDuplicates = false;

  const sanitized = products.map((prod, index) => {
    if (!prod) return prod;
    const currentId = String(prod.id || '').trim();
    if (!currentId || seenIds.has(currentId)) {
      hadDuplicates = true;
      const uniqueId = `${currentId || 'p'}-${Date.now()}-${index + 1}-${Math.random().toString(36).substring(2, 7)}`;
  ...
  return { products: sanitized, hadDuplicates };
}
...
export function getStoredProducts(): Product[] {
  ...
  try {
    ...
    if (saved) {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        // Filter out any permanently deleted products
        const filtered = parsed.filter((p) => p && p.id && !deletedIds.has(String(p.id)));
        const { products: uniqueList, hadDuplicates } = ensureUniqueProductIds(filtered);
        if (hadDuplicates || uniqueList.length !== parsed.length) {
          try {
    ...
    // Check prior storage versions to preserve user-added products
    const previousKeys = [
      'nouvamarket_products_v5',
      'nouvamarket_products_v4',
      'nouvamarket_products_v3',
      'nouvamarket_products_v2',
      'nouvamarket_products_v1',
    ];
      ...
      if (prevSaved) {
        try {
          ...
          if (Array.isArray(prevParsed) && prevParsed.length > 0) {
            const filtered = prevParsed.filter((p) => p && p.id && !deletedIds.has(String(p.id)));
            const { products: uniqueList } = ensureUniqueProductIds(filtered);
            // Save to current key to preserve without loss
            localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(uniqueList));
            return uniqueList;
          }
        } catch {
  ...
  // Default mock products, strictly filtering out any deleted products so they never resurrect
  const filteredMock = MOCK_PRODUCTS.filter((p) => p && p.id && !deletedIds.has(String(p.id)));
  const { products: defaultUnique } = ensureUniqueProductIds(filteredMock);
...
export function saveStoredProducts(products: Product[]): void {
  try {
    const deletedIds = getDeletedProductIds();
    // Exclude any accidentally passed deleted product
    const activeProducts = products.filter((p) => p && p.id && !deletedIds.has(String(p.id)));
    const { products: uniqueProducts } = ensureUniqueProductIds(activeProducts);
    localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(uniqueProducts));
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new Event('products_updated'));
    }

    // Background sync to server so Hostinger and Node backend persist without user data loss
    if (uniqueProducts.length > 0) {
      fetch('/api/products/sync', {
        ...
        console.warn('Background product sync failed (offline or server starting):', err);
      });
    }
  } catch (e) {
...
/**
 * Synchronize local products with server (Hostinger PHP / Node.js)
 * Smart merges without dropping user edits or supplier products, respecting deletions
 */
export async function syncProductsWithServer(): Promise<Product[]> {
  try {
    ...
    const serverList: Product[] = Array.isArray(data) ? data : (data.products || []);
    if (!Array.isArray(serverList)) return local;

    const deletedIds = getDeletedProductIds();
    const map = new Map<string, Product>();

    // 1. Add server products that are not blacklisted
    serverList.forEach((sp) => {
      if (sp && sp.id && !deletedIds.has(String(sp.id))) {
        map.set(String(sp.id), sp);
      }
    });

    // 2. Merge local products (preserving supplier products or custom price/stock)
    let hasLocalOnly = false;
    local.forEach((lp) => {
      if (!lp || !lp.id || deletedIds.has(String(lp.id))) return;
      const pid = String(lp.id);
      if (map.has(pid)) {
        // Keep server product but merge any rich user-customized fields if missing on server
        const existing = map.get(pid)!;
        map.set(pid, {
          ...existing,
          ...lp,
          wholesalePrice: lp.wholesalePrice ?? existing.wholesalePrice,
          suggestedSellingPrice: lp.suggestedSellingPrice ?? existing.suggestedSellingPrice,
          images: (lp.images && lp.images.length > 0) ? lp.images : existing.images,
        });
      } else {
        map.set(pid, lp);
        hasLocalOnly = true;
    ...
    const { products: uniqueList } = ensureUniqueProductIds(merged);
    localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(uniqueList));
    ...
    // If local had items server didn't have, push sync to server
    if (hasLocalOnly && uniqueList.length > 0) {
      fetch('/api/products/sync', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ products: uniqueList }),
      }).catch(() => {});
    }

    return uniqueList;
  } catch (err) {
    console.warn('Could not sync products with server:', err);
